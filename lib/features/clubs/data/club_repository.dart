...
import 'package:turf_app/features/clubs/domain/models/club_member.dart';
import 'package:turf_app/features/clubs/domain/models/club_request.dart';
import 'package:turf_app/features/clubs/domain/models/club_activity.dart';
...
class ClubRepository {
  ...
  Future<List<ClubMember>> getClubMembers(String clubId) async {
    final response = await _supabase
        .from('club_members')
        .select('*, profiles(username, full_name, avatar_url, terra_color, level)')
        .eq('club_id', clubId)
        .order('weekly_distance_km', ascending: false);

    return (response as List).map((m) {
      final json = Map<String, dynamic>.from(m);
      json['profile'] = m['profiles'];
