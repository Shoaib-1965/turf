...
import 'package:turf_app/features/clubs/presentation/providers/club_provider.dart';
  ...
  void _handleLeaveOrJoin() async {
    ...
    if (isOwner) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Owners cannot leave. Transfer ownership or delete club.',
          ),
        ),
      );
      return;
    ...
    try {
      ...
      final membersList = await ref.read(
        clubMembersProvider(widget.club.id).future,
      );
      ...
      if (isMember) {
        ...
        await ref
            ...
            .joinClub(widget.club.id, widget.club.isPublic);
        if (mounted)
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('Joined club')));
      }
                        ...
                        builder: (context) => AlertDialog(
                          ...
                          actions: [
                            TextButton(
                              onPressed: () {
                                Clipboard.setData(
                                  ClipboardData(text: widget.club.inviteCode),
                                );
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                        ...
                        children: [
                          Row(
                            children: [
                              CircleAvatar(
                                radius: 40,
                                backgroundColor: const Color(0xFF1C1C1E),
                                backgroundImage: widget.club.avatarUrl != null
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 4),
                              Text(
                                '${widget.club.totalDistanceKm.toStringAsFixed(1)} km',
                                style: const TextStyle(
                                  color: Colors.white54,
                                  fontSize: 14,
                                ),
                  ...
                  slivers: [
                    ...
                    SliverList(
                      delegate: SliverChildBuilderDelegate((context, index) {
                        ...
                        if (m.role == 'owner')
                          roleColor = const Color(0xFFFFD60A);
                        if (m.role == 'admin')
                          roleColor = const Color(0xFF0A84FF);
            ...
            membersAsync.when(
              ...
              data: (members) {
                final sortedMembers = List.of(members)
                  ..sort(
                    (a, b) => b.weeklyDistanceKm.compareTo(a.weeklyDistanceKm),
                  );
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      trailing: Text(
                        '${m.weeklyDistanceKm.toStringAsFixed(1)} km',
                        style: const TextStyle(
                          color: Color(0xFF00E676),
                          fontWeight: FontWeight.bold,
