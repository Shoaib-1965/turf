...
import 'package:turf_app/features/challenges/domain/models/challenge_participant.dart';
import 'package:turf_app/features/challenges/presentation/providers/challenge_provider.dart';
...
class _MyChallengesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return myAsync.when(
      ...
      error: (e, _) => Center(
        child: Text(
          'Error loading your challenges',
          style: const TextStyle(color: Colors.red),
        ),
      ),
      data: (participants) {
        final activeMyChallenges = participants
            .where(
              (p) =>
                  !p.completed && p.challenge != null && p.challenge!.isActive,
            )
            .toList();
        ...
        if (activeMyChallenges.isEmpty) {
          return const Center(
            child: Text(
              "You haven't joined any active challenges.",
              style: TextStyle(color: Colors.white54),
            ),
          );
        }
...
class _CompletedChallengesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return myAsync.when(
      ...
      error: (e, _) => Center(
        child: Text(
          'Error loading completed challenges',
          style: const TextStyle(color: Colors.red),
        ),
      ),
      data: (participants) {
        final completedChallenges = participants
            .where(
              (p) =>
                  p.completed ||
                  (p.challenge != null && p.challenge!.isCompleted),
            )
            .toList();
...
class ChallengeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = isJoined
        ? (participant!.currentValue / challenge.targetValue).clamp(0.0, 1.0)
        : 0.0;
    ...
    final timeString = isEndsToday
        ? 'Ends today!'
        : '${timeRemaining.inDays} days left';
