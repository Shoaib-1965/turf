...
class ActivityRepository {
  ...
  Future<void> batchInsertLocationPings(List<LocationPing> pings) async {
    ...
    try {
      await _supabase
          .from('location_pings')
          .insert(pings.map((p) => p.toJson()).toList());
    } catch (e) {
  ...
  Future<void> saveActivitySession(ActivitySession session) async {
    try {
      ...
      if (session.xpEarned > 0) {
        await _supabase.rpc(
          'award_xp',
          params: {'p_user_id': session.userId, 'p_xp': session.xpEarned},
        );
      }
      ...
      try {
        final activeParticipants = await _supabase
            ...
            .select(
              'id, current_value, challenge_id, challenges!inner(challenge_type, activity_type, target_value, ends_at)',
            )
        ...
        for (var p in activeParticipants) {
          ...
          final isExpired = DateTime.now().isAfter(
            DateTime.parse(c['ends_at']),
          );
          if (isExpired) continue;

          if (c['activity_type'] == 'any' ||
              c['activity_type'] == session.activityType) {
            double newValue = (p['current_value'] as num).toDouble();

            if (c['challenge_type'] == 'distance') {
              newValue += session.distanceKm;
            } else if (c['challenge_type'] == 'elevation')
              newValue += session.elevationGainM;
            else if (c['challenge_type'] == 'streak')
              newValue += 1; // 1 session = 1 streak count
            else if (c['challenge_type'] == 'speed' &&
                session.avgSpeedKmh > newValue)
              newValue = session.avgSpeedKmh;
            ...
            if (completed) {
              ...
              if (xpReward > 0) {
                await _supabase.rpc(
                  'award_xp',
                  params: {'p_user_id': session.userId, 'p_xp': xpReward},
                );
              }
            }
          }
      ...
      try {
        ...
        for (var goalData in activeGoals) {
          ...
          if (goal.goalType == 'weekly_distance' ||
              goal.goalType == 'monthly_distance') {
            newValue += session.distanceKm;
          } else if (goal.goalType == 'weekly_sessions' ||
          ...
          if (newValue > goal.currentValue) {
            ...
            await _supabase
                .from('fitness_goals')
                .update({'current_value': newValue, 'completed': completed})
                .eq('id', goal.id);
      ...
      try {
        ...
        final clubActivities = (memberships as List)
            .map(
              (m) => {
                'club_id': m['club_id'],
