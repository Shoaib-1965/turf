...
import 'package:turf_app/core/widgets/animated_number.dart';
import 'package:turf_app/core/widgets/user_avatar.dart';
import 'package:turf_app/features/profile/presentation/providers/badge_provider.dart';
...
class _ProfileScreenState extends ConsumerState<ProfileScreen>
  ...
  void initState() {
    ...
    // Clear the notification badge when viewing the profile
                  ...
                  child: Column(
                    children: [
                      ...
                      Padding(
                        ...
                        child: _MyGoalsCard(userId: profile.id),
                      ),
                      const SizedBox(height: 16),
                      // Land Claimed Stats Card
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 16),
                        child: _LandClaimedCard(
                          todayAreaSqm: profile.todayAreaClaimedSqm,
                          totalAreaSqm: profile.totalAreaClaimedSqm,
                        ),
                      ),
                      const SizedBox(height: 16),
                    ],
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Text(
                  '${(session['distance_km'] as num).toStringAsFixed(2)} km ${session['activity_type'].toString().toUpperCase()}',
                  style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                  ),
                ...
                subtitle: Text(
                  '${(session['duration_seconds'] / 60).toStringAsFixed(0)} min',
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  'Captured ${territory['capture_count']} times',
                  style: const TextStyle(color: Colors.white54),
                ),
                trailing: const Icon(
                  Icons.chevron_right,
...
class _MyGoalsCard extends StatelessWidget {
  ...
  const _MyGoalsCard({required this.userId});
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        double progress = 0.0;
        if (activeCount > 0) {
          // Find the nearest goal completion
          double highestProgress = 0.0;
          for (var goal in goals) {
            final target = (goal['target_value'] as num?)?.toDouble() ?? 1.0;
            final current = (goal['current_value'] as num?)?.toDouble() ?? 0.0;
            final p = (current / target).clamp(0.0, 1.0);
            if (p > highestProgress) highestProgress = p;
          }
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.flag, color: Color(0xFF00E676), size: 24),
                const SizedBox(width: 16),
                Expanded(
                  child: Column(
...
class _LandClaimedCard extends StatelessWidget {
  final double todayAreaSqm;
  final double totalAreaSqm;

  const _LandClaimedCard({
    required this.todayAreaSqm,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.landscape, color: Color(0xFF00E676), size: 20),
              SizedBox(width: 8),
              Text(
                'Land Claimed',
                style: TextStyle(
...
class _MyLandTab extends ConsumerWidget {
  final String userId;

  const _MyLandTab({required this.userId});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return FutureBuilder<List<ClaimedTerritory>>(
      future: ref
          .read(territoryClaimRepositoryProvider)
          .fetchUserClaimedTerritories(userId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(
            child: CircularProgressIndicator(color: Color(0xFF00E676)),
        ...
        if (territories.isEmpty) {
          return const EmptyState(
            ...
            title: 'No land claimed yet',
            subtitle: 'Run a closed loop to claim your first territory!',
          );
        }

        // Total area
        final totalArea = territories.fold<double>(
          0,
          (sum, t) => sum + t.areaSqm,
        );
              ...
              (ct) => Card(
                ...
                child: ListTile(
                  leading: Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: _parseColor(ct.colorHex).withOpacity(0.15),
                      shape: BoxShape.circle,
                    ),
                    child: Icon(
                      Icons.landscape,
                      color: _parseColor(ct.colorHex),
                    ),
                  ),
                  title: Text(
                    ct.formattedArea,
